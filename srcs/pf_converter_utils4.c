...
/*   Updated: 2022/01/16 22:05:22 by rodrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	align_left(long long n, long long base, t_spec *spec, char *digits)
{
	...
	ret += putstr_repeat(" ", spec->width - amount_of_digits(n, base));
	return (ret);
}
...
int	align_right(long long n, long long base, t_spec *spec, char *digits)
{
	...
	else if (n < 0)
	{
		...
		if (test_bit(ZERO, spec->flags))
		{
			ret += ft_putchar('-');
			spec->width--;
			ret += putstr_repeat("0", spec->width - amount_of_digits(n, base));
			ret += print_split_sign(n, digits);
		}
		else
		{
			ret += putstr_repeat(" ", spec->width - amount_of_digits(n, base) - 1);
			ret += ft_putchar('-');
			spec->width--;
			ret += print_split_sign(n, digits);
		}
...
/*
**	Prints an integer aligned to the Right (default), adding padding to the
**	left. (It's basically the continuation of the function above, which
...
align_right_pos(long long n, long long base, t_spec *spec, char *digits)
{
	...
	ret = 0;
	if (test_bit(ZERO, spec->flags) || spec->prec > amount_of_digits(n, base))
	{
		if (test_bit(PLUS, spec->flags))
		{
			ret += ft_putchar('+');
	...
	else if (spec->width > amount_of_digits(n, base))
		ret += putstr_repeat(" ", spec->width - amount_of_digits(n, base));
	ret += print_split_sign(n, digits);
	return (ret);
}
